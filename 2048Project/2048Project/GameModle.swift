...
protocol GameModleProtocol:class {
...
class GameModle:NSObject{  //游戏区域的modle层，来记录当前游戏的状态以及提供一些游戏自身的操作等
    
    let dimension:Int
    let threshold:Int
    var gamebord:SequenceGamebord<TileEnum>
    
    unowned let delegate:GameModleProtocol
    ...
    init(dimension:Int,threshold:Int,delegate:GameModleProtocol){
    ...
    func getEmptyPosition()->[(Int,Int)]{
        ...
        return emptyArrys   //遍历SequenceGamebord<TileEnum>，将不为空的位置组成一个(Int , Int)的字典数组返回
    }
    ...
    func insertRandomPositionTile(value:Int){
        ...
        delegate.insertTile(pare: (x,y),value: value)
        
    }
    
    //声明为class的func为类方法，于static相似，但是static方法不能重载，而class声明的类方法可以被override
    ...
    func queenMove(direction:MoveDirection, completion:(Bool)->()){
        ...
        completion(changed)  //实质上的followup函数，如果performmove成功就执行
    }
        ...
        for i in 0..<self.dimension {
            ...
            for order in moveOrders {
                ...
                case let .SINGLEMOVEORDER(s, d, v, m):
                    ...
                    delegate.moveOneTiles(from: (sx, sy), to: (dx, dy), value: v)
                //两个进行合并的
